...
class Hosting extends Component {
    ...
    getBeats = () => {
        firestore
            ...
            .get()
            .then((querySnapshot) => {
                const userBeats = querySnapshot.docs.map((doc) => {
                    return { beatID: doc.id, loop: doc.data() };
                });
    ...
    render() {
        const userDetails = this.state.user ? (
            <p>Hello {this.state.user.displayName.split(" ")[0]}</p>
        ) : (
            <p>Not signed in</p>
        );
        const notLoggedInMessage = this.state.user ? (
            ""
        ) : (
            <p>Sign in to save your loops</p>
        );
